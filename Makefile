...
# Specifying the standard as C99 forces acceptance of the traditional
# (non-ANSI) prototypes and function arguments that m2c uses, but gcc
# does not accept by default anymore.
CFLAGS+= -std=c99
...
COMMONDISTF=$(srcdir)/m2-common.h $(srcdir)/m2-errors.h\
	    ...
	    $(srcdir)/m2-library.h $(srcdir)/m2-version.h
...
install: all uninstall $(libdir) $(bindir) $(man1dir) $(includedir) $(docdir)\
	$(libdir)/$(m2library) $(bindir)/m2c $(man1dir)/m2c.1 $(includedir)/m2lib.h
...
uninstall:
	...
	-rmdir $(libdir) $(bindir) $(man1dir) $(includedir) $(docdir)
...
$(includedir)/m2lib.h:
	$(INSTALLDATA) $(srcdir)/m2lib.h $(includedir)/m2lib.h

clean: modclean
	rm -f $(M2F) m2c $(M2LIBPROC) $(m2library) $(RELEASE_ZIP) $(distdir).tar.gz $(RELEASE_ZIP)
